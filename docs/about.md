# About the Author

<p align="center">
  <img src="../assets/author.png" alt="Vijay Kumar Zharotia" width="300" style="border-radius:12px;box-shadow:0 6px 20px rgba(0,0,0,0.2);">
</p>

<h2 align="center" style="margin-bottom:0;">Vijay Kumar Zharotia</h2>
<p align="center" style="font-size:1.1rem;margin-top:0.3rem;"><strong>Domain, Technology &amp; AI Expert — in a Box</strong></p>

---

Three things rarely live in one person: deep **healthcare domain** knowledge, hands-on **technology** and engineering capability, and applied **AI** expertise. Vijay brings all three — more than 30 years spanning regulated medical-device engineering, large-scale healthcare delivery, and modern agentic AI.

His work spans MedTech, Payer, Provider, and Pharma — leading healthcare and life sciences practices, advising senior executives at marquee accounts, and turning emerging technology into measurable clinical, operational, and commercial outcomes.

## Expert in a box

| Domain | Technology | AI |
|---|---|---|
| MedTech, Payer, Provider, Pharma. Regulation and quality: IEC 62304, ISO 14971, ISO 13485, FDA 21 CFR 820/11, EU MDR 2017/745, HIPAA, CMS-0057-F. | Connected medical devices, FHIR R4 interoperability, EHR / Epic integration, cloud-native platforms, decades of software and systems engineering. | Agentic compliance platforms, clinical RAG, multi-agent orchestration, generative-AI governance, ML / NLP / deep learning. |

## Organizations

Across three decades, Vijay has built and led — from semiconductor and enterprise systems, to medical imaging in Tokyo, to today's regulated healthcare AI — at:

<p align="center" style="font-size:1.05rem;line-height:2;">
<strong>Wipro</strong> · <strong>Aikaki</strong> · <strong>iGATE / Patni Computers</strong> · <strong>Tata IBM</strong> · <strong>EnnovationSoft</strong> · <strong>Infozech</strong>
</p>

## Recognition

- **Patents** — vendor-independent medical-data acquisition; contributor to a secure data-transmission innovation.
- **Author** — this open-source 21-chapter healthcare AI book and its companion [public knowledge graph](https://zkumar.github.io/healthcare-ai-book-graph/).

## Connect

[LinkedIn](https://linkedin.com/in/vijayzharotia){ .md-button .md-button--primary } &nbsp; [Email](mailto:vzharotia@gmail.com){ .md-button }
